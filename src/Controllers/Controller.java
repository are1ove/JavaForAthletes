        ...
        LoginBtn.setOnAction(event -> {
            ...
            try {
                if(!loginText.equals("") && !loginPassword.equals("")) {
                    ...
                    Command command = new Command("login", Arrays.asList(loginText,loginPassword));
                    writer.writeObject(command);
                    writer.flush();
                    Status status = (Status) inFromServer.readObject();
                    String flag = status.getMessage();
                ...
                Alert alert = new Alert(Alert.AlertType.ERROR, "В данный момент серверы приложения недоступны! Повторите попытку позже");
                alert.getDialogPane().getChildren().stream().filter(node -> node instanceof Label).forEach(node -> ((Label)node).setMinHeight(Region.USE_PREF_SIZE));
                alert.show();
